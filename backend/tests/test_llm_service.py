...
Three things are under test here, and none of them needs a language model:
  ...
  - **The request.** TestGeminiPayload asserts the shape Gemini is sent,
    ...
    system channel, everything a person said goes in the user channel. A
    second provider is expected to copy this class.
...
actually built. Nothing here needs a DB either: build_system_prompt() reads one
...
def _domain(name: str = DOMAIN_NAME) -> AgentDomain:
    """An unsaved agent_domains row — build_system_prompt() only reads .name."""
    return AgentDomain(
        name=name,
        description="תיאור",
...
class TestSystemPrompt:
    def test_confines_the_answer_to_the_retrieved_material(self):
        prompt = llm_service.build_system_prompt(_domain())
    ...
    def test_the_domain_it_names_is_the_row_it_was_given(self):
        """Since ABF-120 an agent is a table row, so the subject in rule 4 comes
        from the DB and not from a hardcoded label map. Two domains must not
        get the same prompt."""
        other = llm_service.build_system_prompt(_domain("זכויות אלמנות"))

        assert "זכויות אלמנות" in other
        assert DOMAIN_NAME not in other
...
class TestContextBlock:
    def test_renders_every_chunk_with_its_title_and_source(self):
        ...
        assert "מקור: חוק הסיוע בדיור (https://www.gov.il/housing-aid)" in block
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("ביטוח לאומי", None, "מקור: ביטוח לאומי"),
            (None, "https://example.org/x", "מקור: https://example.org/x"),
            (None, None, None),
        ],
        ids=["name-only", "url-only", "neither"],
    )
    def test_either_half_of_the_provenance_may_be_missing(
        self, source_name, source_url, expected
    ):
        """ABF-120 stores provenance as two nullable columns, so all four
        combinations reach the prompt and each has to render sensibly."""
        chunk = ContextChunk(
            title="כותרת",
            content="תוכן",
        ...
        assert llm_service.render_source_line(chunk) == expected
...
class TestGeminiPayload:
    ...
    def test_the_user_message_stays_in_the_user_channel(self, gemini):
        ...
        # typed is a user turn, never a line appended to the instructions. Any
        # future provider owes the same separation — see the LLMProvider
        # docstring, which a Protocol cannot enforce on its own.
    ...
    def test_an_earlier_turn_stays_in_the_user_channel_too(self, gemini):
        """History is text people typed, so it belongs on the same side of the
        wall as the new question — an injection parked in turn one must not be
        promoted into the rules on turn two."""
        seen = self._capture(
            gemini,
            conversation_history=[
                HistoryTurn(role=AgentMessageRole.USER, content=INJECTION_MESSAGE)
            ],
        )

        system_text = " ".join(
            part["text"] for part in seen["body"]["systemInstruction"]["parts"]
        )
        assert INJECTION_MESSAGE not in system_text
        assert seen["body"]["contents"][0]["parts"][0]["text"] == INJECTION_MESSAGE
