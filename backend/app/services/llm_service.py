...
class ContextChunk:
    ...
    where it can be checked. Both are optional: some passages are written
    in-house by the association and cite nothing.
    """
...
class LLMProvider(Protocol):
    ...
    **One requirement a Protocol cannot express, and every provider owes:**
    `system_prompt` and `context_chunks` are ours and must travel in the
    provider's system/instruction channel; `user_message` and
    `conversation_history` are things people typed and must travel in its
    user/message channel. A provider that concatenates all four into one
    string still type-checks and still returns an answer — and quietly
    removes the structural half of the injection defence, leaving only the
    prompt's own request not to obey embedded instructions. See
    GeminiProvider._build_payload(), and the per-provider assertion in
    tests/test_llm_service.py::TestGeminiPayload that a new provider is
    expected to copy.
    """
...
#: is *part of the stored answer* — one row of agent_messages, encrypted with
#: the rest of it, replayed verbatim by ABF-123 and stripped back off by
#: _without_disclaimer() before the next prompt. The model is instructed to
#: answer in Hebrew, so a disclaimer resolved into another language would sit
#: under a Hebrew paragraph mid-sentence.
ANSWER_DISCLAIMER = (
...
#: material was found but does not cover the question. Hebrew for the same
#: reason as ANSWER_DISCLAIMER: it is stored as the agent's turn.
NO_CONTEXT_ANSWER = (
...
def build_system_prompt(domain: AgentDomain) -> str:
    ...
    Takes the AgentDomain row rather than an enum member (there is no agent
    enum since ABF-120 — a domain is a table row an admin can add) and reads
    ``domain.name`` for rule 4. Built per domain rather than stored as a
    constant because that rule names the subject: the agent for single-parent
    rights must decline a question about another agent's subject instead of
    answering it from the wrong knowledge base.

    ``domain.name`` is staff-authored content, and it lands in the system
...
def render_source_line(chunk: ContextChunk) -> str | None:
    """The provenance line for one passage, or None when it cites nothing.

    ABF-120 splits provenance into a readable name and a checkable URL, and
    either may be missing. The name leads when both are present — it is what
    the model is asked to cite in the answer body (rule 5) — with the URL in
    brackets after it. A passage with only a URL still gets a line, because a
    link the reader can open is better provenance than none.
    """
    name, url = chunk.source_name, chunk.source_url
    if name and url:
        return f"מקור: {name} ({url})"
    if name:
        return f"מקור: {name}"
    if url:
        return f"מקור: {url}"
...
def render_context_block(context_chunks: Sequence[ContextChunk]) -> str:
    ...
    for index, chunk in enumerate(context_chunks, start=1):
        ...
        source_line = render_source_line(chunk)
        if source_line:
            header += f"\n{source_line}"
...
class GeminiProvider:
    ...
    def _build_payload(
        ...
        * ``contents`` carries only things people said — the earlier turns of
        ...
        Every provider owes this split; see the LLMProvider docstring.
        """
...
def get_provider() -> LLMProvider:
    ...
    Raises LLMNotConfiguredError for an unknown name, which agent_service
    turns into a 503 — a typo in an environment variable is a deployment
    fault, and must not degrade into a referral answer that would claim the
    knowledge base has nothing when it does.
    """
