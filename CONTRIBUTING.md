...
- **פרמטר** — `translate("some.key", count=7)`. שתי השפות חייבות להחזיק את אותו
  `{placeholder}`; יש בדיקה שמשווה ביניהן. ההודעה היחידה כזאת בקטלוג היא
  `agents.rate_limited`, שמקבלת את `AGENT_RATE_LIMIT_PER_DAY` — מספר שהוא הגדרה,
  ולכן אי אפשר לכתוב אותו לתוך אף אחד משני התרגומים.
...
`agent_service` ו-`llm_service` כששבה תשתית הסוכן (ABF-122), כפוף לכלל הזה מהיום שהוא
נוסף, בלי שאיש צריך לזכור לרשום אותו כאן.

**חריג אחד, והוא לא באמת חריג:** `ANSWER_DISCLAIMER` ו-`NO_CONTEXT_ANSWER`
ב-`llm_service.py` נשארות עברית קשיחה. הן אינן הודעה *על* בקשה אלא **חלק מתשובת
הסוכן** — נשמרות מוצפנות בשורת `agent_messages` ומוצגות שוב חודשים אחר כך. שורה
שמורה אינה יכולה לעקוב אחרי ה-`Accept-Language` של קורא/ת עתידי/ת, ותרגום בזמן
הכתיבה היה מדביק פסקה באנגלית לסוף תשובה בעברית. הן גם אינן באף אחת משלוש הצורות
שהסריקה מחפשת, ולכן אין צורך בפטור.
...
**חריג מאושר — סדרת ה-i18n.** טיקטי התרגום (ABF-127 עד ABF-137)
...
מיגרציות ה-frontend (ABF-127…ABF-136) הן PRs מוערמים (stacked) שכל אחד מהם
ממשיך את קודמו, והצורה האחידה הזאת היא מה שמראה את השרשרת ברשימת הענפים.
ABF-137 (`Accept-Language` ב-backend) שייך לאותו מאמץ ולכן לאותה צורה, אבל
מסונף מ-`main` ואינו חלק מהערימה — אין לו קובץ משותף איתה.

זה החריג היחיד; כל ענף אחר — כולל טיקט i18n חדש שאינו חלק מהמאמץ הזה —
לוקח קידומת סוג.
